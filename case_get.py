...
def is_Method(method,url,headers,payload):
    if method == "GET":
        payload = json.loads(dict["payload"])
...
if __name__ == "__main__":
    data_path = "/Users/hupc/Desktop/case.xlsx"
    sheetname = "case_list"
    get_data = ExcelData(data_path, sheetname)
    datas = get_data.readExcel()
    for dict in datas:
        method = dict["Request_Method"]
        # print(method)
        url = dict["url"]
        # print(url)
        headers = json.loads(dict["header"])
        # print(headers)
        payload = dict["payload"]
        is_Method(method, url, headers, payload)
